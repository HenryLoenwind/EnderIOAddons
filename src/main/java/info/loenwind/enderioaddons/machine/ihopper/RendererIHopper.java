...
public class RendererIHopper implements ISimpleBlockRenderingHandler {
  ...
  private static BoundingBox bb(int minX, int minY, int minZ, int maxX, int maxY, int maxZ) {
    return new BoundingBox(px * minX, px * minY, px * minZ, px * maxX, px * maxY, px * maxZ);
  }
  ...
  private void renderSubBlock(int x, int y, int z, @Nonnull AbstractMachineEntity te, float[] brightnessPerSide, @Nonnull TankSlot tankSlot) {
    ...
    // top box
    BoundingBox bb1 = translateIntoSlot(bb(0, 10, 0, 16, 16, 16), pos, x, y, z);
    ...
    // inside
    BoundingBox bb2 = translateIntoSlot(bb(2, 10, 2, 14, 16, 14), pos, x, y, z);
    for (ForgeDirection dir : AROUND) {
    ...
    // center box
    BoundingBox bb3 = translateIntoSlot(bb(4, 4, 4, 12, 10, 12), pos, x, y, z);
    for (ForgeDirection dir : AROUND) {
    ...
    BoundingBox bb4 = translateIntoSlot(bb(x0, x1, y0, y1, z0, z1), pos, x, y, z);
  ...
  private BoundingBox translateIntoSlot(BoundingBox bb, int[] pos, int x, int y, int z) {
